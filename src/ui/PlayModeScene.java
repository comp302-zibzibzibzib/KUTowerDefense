...
public class PlayModeScene extends AnimationTimer {
	...
	private void showLotCircle(ImageView lot) {
		...
		archerButton.setOnMouseClicked(event -> {
			removeCircle(towerSelection);
			if (!TowerController.canBuildArcher())
				return;
		...
		mageButton.setOnMouseClicked(event -> {
			removeCircle(towerSelection);
			if (!TowerController.canBuildMage())
				return;
		...
		artilleryButton.setOnMouseClicked(event -> {
			removeCircle(towerSelection);
			if (!TowerController.canBuildArtillery())
				return;
	...
	private void playerStatsPutter() {
		...
		Label lives = new Label(
				String.format("%d/%d", PlayerController.getPlayerLives(), GameOptionsController.getStartingLives()));
		Label waves = new Label(
				String.format("%d/%d", PlayerController.getWaveNumber(), GameOptionsController.getNumberOfWaves()));
	...
	public void handle(long arg0) {
		...
		for (int i = 0; i < EntityController.getNumberOfEnemies(); i++) {
			if (!EntityController.isEnemyInitialized(i))
				continue;

			String type = null;
			if (EntityController.isGoblin(i))
				type = "goblin";
			if (EntityController.isKnight(i))
				type = "warrior";
