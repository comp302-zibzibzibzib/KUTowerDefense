...
public class Map implements Serializable {
	// Overview: This class represents game world in which towers can be placed on
	// lots, enemies move through a path
	// and projectiles go towards enemies
	// Map objects can be saved and changed through the Map Editor
	// A typical map consists 16x9 tiles which at least 4 of them are lot tiles, a
	// starting tile at the bottom of the map, an ending tile at the top of the Map
	// and a single connected series of pathTiles that start from the starting tile
	// and end at ending tile

	// Abstract Invariant: Starting and Ending tiles Must not be the same, starting
	// tile is bottom of the map
	// ending tile is at the top of the map, single connected lines of path tiles
	// connect starting and ending tiles
	...
	// each path tile is only connected to two other tiles (except starting and
	// ending tiles)
	...
	// the start tile is located at tileMap[height - 1][x] for some x in between [0,
	// 1, 2... ,width-1]
	// the end tile is located at grid[0][y] for some y in between [0, 1, 2...
	// ,width-1]
	// there are at least 4 empty lot tiles
	// there is a unique sequence of adjacent (N/S/E/W) pathTiles that form an
	// unbranching path
	...
	// Starting tile must be somewhere at the bottom end ending tile must be
	...
	public Map(String mapName, PathTile startingTile, PathTile endingTiles, int height, int width) {
		...
		if (true)
			return; // This is to trick the compiler so we don't have to delete the code below but
					// also never use it
		// DEPRICATED BELOW!!!

		/*
		 * Tile prevStart =
		 * tileMap[(int)startingTile.location.yCoord][(int)startingTile.location.xCoord]
		 * ; Tile prevEnd =
		 * tileMap[(int)endingTile.location.yCoord][(int)endingTile.location.xCoord];
		 * //Ensure that both starting and ending tiles are of type PATH if
		 * (startingTile.getType() != TileType.PATH || endingTile.getType() !=
		 * TileType.PATH) return; // Raise an invalid argument exception?
		 * 
		 * this.startingTile = (startingTile.location.yCoord == height-1) ? startingTile
		 * : null; //Display an error message in UI??? this.endingTile =
		 * (endingTile.location.yCoord == 0) ? endingTile : null;
		 * 
		 * tileMap[(int)startingTile.location.yCoord][(int)startingTile.location.xCoord]
		 * = this.startingTile;
		 * tileMap[(int)endingTile.location.yCoord][(int)endingTile.location.xCoord] =
	...
	public boolean repOK() {
		// I can't be bothered to write a repOK for the new map with multiple starting
		// and ending tiles
		// If you are grading this, I suggest you look at the junit-Group or
		// junit-<team-member-name> branches
		return true;
	}
	...
	private double[] calculateBankingOffset(double[] incomingDir, double[] outgoingDir) {
		...
		// When going right and turning down: offset should be {-0.2, -0.2} (slightly
		// left and up from center)
		// When going right and turning up: offset should be {-0.2, 0.2} (slightly left
		// and down from center)
		// When going left and turning down: offset should be {0.2, -0.2} (slightly
		// right and up from center)
		// When going left and turning up: offset should be {0.2, 0.2} (slightly right
		// and down from center)
	...
	public static double[] getEdgeTargetOffset(PathTile edge) {
		...
		double offset = Tile.tileLength * 0.25;
		double[] offsetPercentage = edge.getPathType().getPathOffsetPercentage();
		double cX = offsetPercentage[0] * Tile.tileLength;
		double cY = offsetPercentage[1] * Tile.tileLength;

		if (map.getStartingTiles().contains(edge) && path != null && path.size() >= 2) {
			double[] dir = getDirectionVector(edge, path.get(1));
			double sX = -dir[0] * offset;
			double sY = -dir[1] * offset;
			return new double[] {cX + sX, cY + sY};
		}
		...
		if (map.getEndingTiles().contains(edge)) {
			...
			if (path != null && path.size() >= 2) {
				...
				double dX = dir[0] * offset;
				double dY = dir[1] * offset;
				return new double[] {cX*0.80 + dX, cY*0.80 + dY};
			}
		}
	...
	public static int[] locationToTileMap(Location location) {
		...
		// EFFECTS: Returns the tile map index corresponding to a domain Location
		// coordinate
		int x;
		int y;
