...
public class MapEditor implements Serializable {
	...
	public boolean isValidMap() {
		...
		if (isStartingTilePlaced && isEndingTilePlaced && map.getStartingTile() != null
			...
			// Ensure that there is a connected path and, first tile and last tiles are
			// starting and ending tiles respectively. (Should work if A* is implemented
			// correctly)
			if (reachablePath.getFirst().equals(map.getStartingTile())
					&& reachablePath.get(reachablePath.size() - 1).equals(map.getEndingTile())) {
